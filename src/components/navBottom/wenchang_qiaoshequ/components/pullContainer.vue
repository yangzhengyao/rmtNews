...
export default {
  ...
  methods: {
    ...mapMutations("wenchang_qiaoshequ", [
      "set_currentContent",
      "set_indexPage",
      "set_indexLocation"
    ]),
    ...
    loadTopAjax() {
      ...
      var that = this;
      var uuid = this.indexActive ? this.type : null;
      this.classPage = 0;
      // this.loading = true
      this.get_listItem_data({page: this.classPage, uuid: uuid ,userId:this.userDataid})
        ...
        .catch(err => {
          if (this.contentJson.length > 0) {
            ...
            this.get_listItem_cache().then(cache => {
              if (cache) {
    ...
    loadBottomAjax() {
      ...
      if (!this.bottomNoData) {
        ...
        this.get_listItem_data({page: this.classPage, uuid: uuid ,userId:this.userDataid}).then(
          res => {
            var tempArr = [];
            res.data[0].list.forEach(function(item, i) {
              tempArr.push({
