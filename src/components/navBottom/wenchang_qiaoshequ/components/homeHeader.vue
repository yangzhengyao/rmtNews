<template>
    <div id="indexHeader">
        ...
        <nav class="homeNavFixed">
            <div class="nav_ul nav_qinqiang nav_wenlv">
...
export default {
    ...
    methods: {
        ...
        slideTo (index) {
            this.$nextTick(() => {
                let _container = $('.nav_ul.nav_qinqiang')           // 获取滚动容器元素
                let _column = $('.nav_ul.nav_qinqiang a').eq(index)  // 获取当前active栏目元素
                if (_column) {
    ...
    activated () {
        this.$nextTick(() => {
            $('.nav_ul.nav_qinqiang').scrollLeft(this.move)
        })
    }
}
</script>
