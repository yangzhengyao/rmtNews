    ...
    <div id="indexHeader">
        ...
        <nav class="homeNavFixed navCommuntiy">
            <div class="nav_ul newCommunity ">
                <a href='javascript:;' v-for="(item,index) in indexColumn" :class='{active: indexActive == item.classpath}' @click="navClick(item.classpath,index)" :key="index">{{apis.getTheme() =='dongpo' && item.classname=='活动'?'报料':item.classname}}
                    <span></span>
                </a>
            </div>
        </nav>
