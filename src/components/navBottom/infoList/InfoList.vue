<template>
    <div class="listBox">
        <div class="infoList" v-if="islnfoListIn" v-for="(item,index) in infoList" :key="index"> <!-- data中控制遍历数组 -->
            <div class="infoHeader">
                <img @click="jumpUserInfo(item.user_id)" :src='item.user.imgurl ? apis.theme.imgHost+item.user.imgurl : imgHead' alt="" class="headerImg"> 
                <dl>
                    <dt @click="jumpUserInfo(item.user_id)">{{item.user.personName}}</dt>
                    <dd>{{item.pub_time}}</dd>
                </dl>
                <div class="floatR"  v-if="isShoot != 2">
            ...
            <div @click.stop="routeDateil(item.auto_id)" @touchmove="touchmoveFunc()">
                ...
                <div class="listLoca" v-if="apis.getTheme() !='dongpo'">
                    <div v-if="isShoot != 2">
                    ...
                    <div class="sment" @click="jumpSment"  v-if="!isShare && apis.getTheme() =='mile'">
                        免责声明
                    </div>
                </div>
            </div>
        ...
        <div class="zanWu" v-if="!islnfoListIn">没有更多的内容了</div>
    </div>
</template>
<script>
...
export default {
    ...
    methods: {
        jumpUserInfo(userId){
            if(this.isShoot == 2){   //拍客不能查看他人主页
                return;
            }
            this.apis.urlParams('detailsPage?id='+userId+'&type=0');
        },
        jumpSment(){
            this.apis.urlParams('statement');
        },
        ...
        routeDateil(autoId){
            if(this.isShare == false || this.isShoot == 2){
                return;
            }
            if(this.userDataid){
                var postData = {
        ...
        delready(ind,auto_id){
            Bus.$emit('childDel',{index:ind,autoId:auto_id});
        },
        getDataid(){
			let userDataid = JSON.parse(localStorage.getItem("userData"));
			if( userDataid !== null){
    ...
    created(){
        Bus.$on('stateList',(val) =>{
            this.infoList = val;
            this.showGallery= [];
            // var data = this.infoList[0].blog_info_videos
            // if(typeof(data) == 'string' && data){
            //     data = JSON.parse(this.infoList[0].blog_info_videos)
            //     this.infoList[0].blog_info_videos = JSON.parse(data)
            // }
            this.infoList.forEach((v,i) => {
                var data = v.blog_info_videos
                if(typeof(data) == 'string' && data){
                    data = JSON.parse(v.blog_info_videos)
            ...
            if(val.length <= 0){
                this.islnfoListIn = false;
            }else{
                this.islnfoListIn = true;
            }
        ...
        Bus.$on('islnfoListIn',(val) =>{
            this.islnfoListIn = val;
        });
    }
}
</script>
<style lang='less' scoped>
    ...
    .sment{
        width: 2rem;
        height: .56rem;
        // background-image: url(../../img/v3/mzsm.png);
        background: #eaeef9;
        position: absolute;
        color: #576b95;
        top: -9px;
