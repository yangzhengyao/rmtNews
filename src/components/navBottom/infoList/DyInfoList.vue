<template>
    <div class="listBox">
        <div class="infoList" v-if="islnfoListDy" v-for="(item,index) in infoList" :key="index"> <!-- data中控制遍历数组 -->
            <div class="infoHeader">
                <!-- apis.urlParams('detailsPage?id='+item.user_id+'&type=0') -->
                <img @click="apis.urlParams('detailsPage?id='+item.user_id+'&type=0')" :src='item.user.imgurl ? apis.theme.imgHost+item.user.imgurl : imgHead' alt="" class="headerImg" @touchmove="touchmoveFunc()">
                <dl>
                    <dt @click="apis.urlParams('detailsPage?id='+item.user_id+'&type=0')" @touchmove="touchmoveFunc()">{{item.user.personName}}</dt>
                    <dd>{{item.pub_time}}</dd>
                </dl>
                <div class="floatR" v-if="apis.getTheme() !='dongpo'">
                    ...
                    <div class="delImg" v-if="isDele == 2 || item.user_id == userDataid">
        ...
        <div class="zanWu" v-if="!islnfoListDy">没有更多的内容了</div>
    </div>
</template>
<script>
...
export default {
    ...
    created(){
        ...
        Bus.$on('islnfoListDy',(val) =>{
            this.islnfoListDy = val;
        });
    }
}
</script>
