<template>
    <div class="listBox">
        <div class="infoList" v-if="islnfoListCir" v-for="(item,index) in infoList" :key="index"> <!-- data中控制遍历数组 -->
            ...
            <div @click.stop="routeDateil(item.auto_id)" @touchmove="touchmoveFunc()">
                ...
                <div class="listImg">
                    ...
                    <!-- <div :style="{'background-image':'url('+i.images+')'}" @click.stop="bigImg(ind,JSON.parse(item.blog_info_images))" :alt="i.ind" v-for="(i,ind) in JSON.parse(item.blog_info_images)" :key="ind" :class="JSON.parse(item.blog_info_images).length == 1 ? 'zoomPic':''">
                    </div> -->
        ...
        <div class="zanWu" v-if="!islnfoListCir">没有更多的内容了</div>
    </div>
</template>
<script>
...
export default {
    ...
    created(){
        ...
        Bus.$on('islnfoListCir',(val) =>{
            this.islnfoListCir = val;
        });
    }
}
</script>
