<template>
    <div class="listBox">
        <div class="infoList" v-if="islnfoListSubSp" v-for="(item,index) in infoList" :key="index"> <!-- data中控制遍历数组 -->
            ...
            <div @click.stop="routeDateil(item)" @touchmove="touchmoveFunc()">
                ...
                <div class="listImg">
                    ...
                    <div class="listChi" v-lazy:background-image="i"  @click.stop="bigImg(ind,item.subImages)" :alt="i.ind" v-for="(i,ind) in item.subImages" :key="ind" :class="item.subImages.length == 1 ? 'zoomPic':''">
                    </div>
        ...
        <div v-if="!islnfoListSubSp || infoList.length == 0" class="zanWu">
            <div></div>
            <p>您还没有订阅内容</p>
        </div>
    </div>
...
export default {
    ...
    methods: {
        ...
        linkTo( index, event, params, item ){
    ...
    created(){
        ...
        Bus.$on('islnfoListSubSp',(val) => {
            this.islnfoListSubSp = val;
        })
    }
}
</script>
