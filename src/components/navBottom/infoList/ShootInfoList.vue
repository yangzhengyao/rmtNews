<template>
    <div class="listBox">
        <div class="infoList" v-if="islnfoListShoot" v-for="(item,index) in shInfoList" :key="index"> <!-- data中控制遍历数组 -->
            <div class="infoHeader" @click.stop="routeDateil(item.auto_id)">
                <!-- apis.urlParams('detailsPage?id='+item.user_id+'&type=0') -->
                <img :src='item.user.imgurl ? apis.theme.imgHost+item.user.imgurl : imgHead' alt="" class="headerImg" @touchmove="touchmoveFunc()">
                <dl>
                    <dt @touchmove="touchmoveFunc()">{{item.user.personName}}</dt>
                    <dd>{{item.pub_time}}</dd>
                </dl>
        ...
        <div class="zanWu" v-if="!islnfoListShoot">没有更多的内容了</div>
    </div>
</template>
<script>
    ...
    methods: {
            ...
            //判断用户下次点击是添加收藏还是取消收藏
                ...
                .then( res => {
                    if( res.status ){
                        ...
                        this.shInfoList[index].fav = !this.shInfoList[index].fav;
                    } 
                })
            }
        },
        ...
        cancelFoc(index,user_id){
            if( localStorage.getItem( 'userData' ) === null ){
                ...
                var focus = this.shInfoList[index].focus;
                for(var i = 0; i < this.shInfoList.length; i++) {
                    if(this.shInfoList[i].user_id == user_id) {
                        this.shInfoList[i].focus = !this.shInfoList[i].focus;
                        // break;
                    }
                }
                this.userId = user_id;
                this.addUserFans(this.shInfoList[index].focus);
        ...
        cancel(like,index,autoId,themeId, topicId){
            ...
            let userDataid=JSON.parse(localStorage.getItem("userData"));
            this.initParams=userDataid.user_id;
            this.shInfoList[index].like = !this.shInfoList[index].like;
            if(!this.shInfoList[index].like){
                this.shInfoList[index].likes_num--;
                if(this.shInfoList[index].likes_num == 0){
                    this.shInfoList[index].likes_num = null;
                }
                this.canclelike(autoId, themeId, topicId);
            }else{
                this.shInfoList[index].likes_num++;
    ...
    created(){
        Bus.$on('stateListShoot',(val) =>{
            ...
            this.infoList.forEach((v,i) => {
                ...
                if(v.hasOwnProperty("appInfoId")){
                    this.shInfoList.push(v)
                }
            })
        ...
        Bus.$on('islnfoListShoot',(val) =>{
            this.islnfoListShoot = val;
        });
    }
}
</script>
