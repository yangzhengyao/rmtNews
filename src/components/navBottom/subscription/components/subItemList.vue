...
<div>
  <ul class="listItem" :class="{'checkBox' : visible,'listItemMargin':isHaixia}" @scroll="handleScroll">
    ...
    <div v-for="(section,index) in itemJson" :key="index">
         <!-- 置顶稿件 -->
        <li  v-if=" section.hasOwnProperty('isTop') &&  section.isTop && indexActive == apis.theme.news.columnId.tuijian" @click.stop="saveData(section)" @touchmove="touchmoveFunc()" class="small_zhiding">
        ...
        <!-- 视频小图 -->
        <li v-else-if="section.playonlineurl" @click.stop="saveData(section)" @touchmove="touchmoveFunc()" class="small">
            <div class="small_left">
                <div class="title">
                    <h3>{{ section.title }}</h3>
                </div>
        ...
        >
          ...
          <div class="small_right" v-lazy:background-image="section.ptitlepic" :key="section.ptitlepic">
