<template>
    <div id="indexHeader">
        ...
        <!-- 海沧两岸一家亲 -->
        <!-- <li style="height: 2.41rem; background: transparent; box-shadow: none;padding:0.2rem .2rem;background:#fff">
            <img style="width:3.45rem;height: 100%;float:left;box-shadow: 1px 1px 3px 0px #c1c1c1;"  :src='"~@/assets/img/"+apis.getTheme()+"/shequ.png"' @click="urlActivity">
            <img style="width:3.45rem;height: 100%;float:right;box-shadow: 1px 1px 3px 0px #c1c1c1;" :src='"~@/assets/img/"+apis.getTheme()+"/taibao.png"' @click="apis.urlParams('twoCommunity')">
        </li> -->
    </div>
</template>
<script>
...
export default {
    ...
    methods: {
        ...
        slideTo (index) {
            this.$nextTick(() => {
                let _container = $('.nav_ul.nav_haicang')           // 获取滚动容器元素
                let _column = $('.nav_ul.nav_haicang a').eq(index)  // 获取当前active栏目元素
                if (_column) {
    ...
    activated () {
        this.$nextTick(() => {
            $('.nav_ul.nav_haicang').scrollLeft(this.move)
        })
    }
}
</script>
        ...
        .nav_ul {
            ...
            a {
                ...
                *display:inline-block;
                ...
                padding:.2rem 1.08rem; //0 0.267rem .3rem .39rem .3rem 0
