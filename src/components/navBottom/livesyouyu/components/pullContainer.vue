<template>
    ...
    <div class="container swiper-box-special" :class="type" v-infinite-scroll="loadBottomAjax" infinite-scroll-disabled="bottomLock" infinite-scroll-distance="10" infinite-scroll-immediate-check="false">
        ...
        <!-- <mt-loadmore :top-method="loadTopAjax" @top-status-change="handleTopChange" ref="loadmore" :auto-fill='false' :distance='indexSwiper'> -->
            ...
            <!-- listItem --> 
            <youyulist :itemJson="contentJson"  :itemJsons="contentJsons"  :itemJsonsnew="contentJsonsnew"   :typeid="type"  v-if='contentJson.length > 0'></youyulist>
            <!-- 底部提示 -->
...
export default {
    ...
    methods: {
        ...
        ...mapActions('index', [
            'get_listItem_cache',
            'lives_listItem_data',
            'lives_listItem_datajvzhen',
            'lives_listItem_datajvzhennew',
            'get_stick_data',
            'get_banner_data'
        ]),
        init () {
        ...
        loadTopAjax () {
            ...
            this.classPage=0
            this.loading = true
            this.lives_listItem_data(this.classPage)
            .then(res => {
                this.loading = false   
                var tempArr=[];
                var bannerArr=[],numTemp=0;
                console.log( res.data,"单身但是多所多所多")
      ...
      this.lives_listItem_datajvzhen(this.classPage)
            .then(res => {
                var tempArr=[];
                var bannerArr=[],numTemp=0;
                 res.data[0].list.forEach(function (item,i) {
 ...
 this.lives_listItem_datajvzhennew(this.classPage)
            .then(res => {
                var tempArr=[];
                var bannerArr=[],numTemp=0;
                res.data[0].list.forEach(function (item,i) {
