<template>
<div class="listbig">
     <div class="spaper" v-if='newsID=="5d0312ace4b0ed2aef82fdaa"'>
            <ul>
                <li v-for="(sectionlist,index) in itemJsonlist" :key="index" @click.stop='saveData(sectionlist)'>
                <img :src='sectionlist.titlepic'/>
                <p>{{sectionlist.title}}</p>
                </li>
            </ul>
     </div>
    <div class="spaper" v-else-if='radiobroadcastid=="5d031292e4b0ed2aef82fda8"'>
            <ul>
                <li v-for="(sectionlist,index) in itemJsonlist" :key="index" @click.stop='saveData(sectionlist)'>
                <div class="section-opents"><img src='../assets/img/tv.png' /></div>
                <p>{{sectionlist.title}}</p>
                </li>
            </ul>
    </div>
   <ul class="listItem" :class="{'checkBox' : visible,'listItemMargin':isHaixia}" :style="{'background': typeid == '5cc7b1360cf2acaeba3f52ae' ? '#fff' : ''}">
         <!-- {{ itemJson }}  -->
         <!-- <div class="topbigtitle" v-if='typeid==="5cc7b1360cf2acaeba3f52ae"'>右玉媒体矩阵</div> -->
        ...
        <div v-for="(section,index) in itemJson" :key="index">
            ...
            <!-- 右玉媒体矩阵 -->
          <div v-if='typeid==="5cc7b1360cf2acaeba3f52ae"' class="boxall bottomsnew">
               <div class="" v-if="section.author=='1'" @click="apis.urlParams('wechatItem',section,false)">
                    <div class="imgnew-boxnew" :style="{'background-image':'url('+section.titlepic+')',}"></div>
                    <div class="imgnew-title">{{section.title}}</div>
               </div>
              <div class="" v-else-if="section.author=='2'" @click="apis.urlParams('newspapers',section,false)">
                    <div class="imgnew-boxnew" :style="{'background-image':'url('+section.titlepic+')',}"></div>
                    <div class="imgnew-title">{{section.title}}</div>
               </div>
                 <div class="" v-else-if="section.author=='3'" @click="apis.urlParams('tvstations',section,false)">
                    <div class="imgnew-boxnew" :style="{'background-image':'url('+section.titlepic+')',}"></div>
                    <div class="imgnew-title">{{section.title}}</div>
               </div>
                <div class="" v-else-if="section.author=='4'" @click="apis.urlParams('radiobroadcasts',section,false)">
                    <div class="imgnew-boxnew" :style="{'background-image':'url('+section.titlepic+')',}"></div>
                    <div class="imgnew-title">{{section.title}}</div>
               </div>
               <div class=""  @click="urlhtml(section.href)" v-else>
                    <div class="imgnew-boxnew" :style="{'background-image':'url('+section.titlepic+')',}"></div>
                    <div class="imgnew-title">{{section.title}}</div>
               </div>
          </div> 
            <li v-else-if="section.lives" @click.stop='saveData(section)' @touchmove="touchmoveFunc()" class="video-list lives-list" style="margin-top:10px">
                <!-- checkBox -->
     ...
     <div v-if='typeid=="5cc7b1360cf2acaeba3f52ae"' class="bottomslist">
           <div class="bigtitle">中央省市媒体矩阵</div>
           <div  v-for="(section,index) in itemJsonsnew" :key="index"  class="boxall" @click="urlhtmlnew(section.href)">
               <div class="imgnew-boxnew" :style="{'background-image':'url('+section.titlepic+')',}"></div>
               <div class="imgnew-title">{{section.title}}</div>
            </div>
        </div>
    <!-- 部门、乡镇新媒体矩阵 -->
        <div v-if='typeid=="5cc7b1360cf2acaeba3f52ae"' class="bottomslist">
           <div class="bigtitle">部门、乡镇新媒体矩阵</div>
           <div  v-for="(section,index) in itemJsons" :key="index"  class="boxall" @click="apis.urlParams('wechatItem',section,false)">
               <div class="imgnew-boxnew" :style="{'background-image':'url('+section.titlepic+')',}"></div>
               <div class="imgnew-title">{{section.title}}</div>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        saveData(section) {
            console.log(section,"sectionsectionsectionsectionsection")
            let that = this;
         if(section.lives){
                ...
                if( section.properties != undefined && section.hasOwnProperty('properties') ){
                    if( section.properties.hasOwnProperty('isVr') == true && section.properties.isVr == '1'){
                        if( plus.os.name == 'iOS' ){
                            ...
                            //获取java辅助类
                            var Helper = plus.android.importClass("com.zkwg.pynews.Helper");
                            //创建对象实例
    ...
    //   console.log(this.itemJson,"itemJsonitemJsonitemJson")
    
    },
    mounted() {
        // this.videoImgFunc()
        //  console.log(this.typeid,"dddd")
...
.listbig .boxall{ width: 26%; overflow: hidden; float: left; margin-left: 5.6%; margin-top: 2%; height: 2.8rem;}
.listbig .imgnew-boxnew{width:100%;height:0;padding-bottom: 100%;overflow:hidden;background-position: center center;background-repeat: no-repeat;background-size:56%;}
.listbig .imgnew-title{ font-size: 0.27rem; color: #505050; text-align: center;}
.listbig .bottomLoad{ clear: both;}
.listbig .listItem{ clear: both; display: block; margin-bottom: 0.3rem; overflow: hidden;}
.listbig .bottomslist{ clear: both; width: 100%; overflow: hidden; background: #fff; padding-bottom:0.3rem; }
.listbig .bottomslist .bigtitle{ font-size: 0.3rem; margin: 0.3rem 0 0 0.5rem;  color: #505050}
.listbig .topbigtitle{ font-size: 0.3rem; margin: 0.3rem 0 0 0.5rem;  color: #505050; margin-top: 20px; margin-bottom: 10px}
.listbig .bottomslist { margin-bottom: 0.3rem}
.listbig .spaper{width:100%; overflow: hidden; margin-top:5%}
.listbig .spaper ul li{ width:44%; float:left; margin-left:0.3rem; margin-bottom:0.3rem; text-align:center;height: 100%; }
.listbig .spaper ul li img{width:100%}
.listbig .spaper ul li p{ font-size:0.24rem; margin-top:0.2rem}
.listbig .section-opents{border:1px solid #e5e5e5;padding-top:0.2rem}
.listbig .section-opents img{width:1.16rem!important;height:0.96rem;}
</style>
