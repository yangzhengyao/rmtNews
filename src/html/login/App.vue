      ...
      <div class="wrapper">
        ...
        <div class="defaultLogin">
          ...
          <div class="toggleLogin">
            <span @click="toggleFn" style="color:#5a5a5a">{{ userPwd ? '验证码登录':'用户密码登录' }}</span>
            <span @click="register" style="color:#5a5a5a">新用户注册</span>
          </div>
  ...
  methods: {
    ...
    wechatLogin( openid,nickname,headimgurl,sex,language,city,province,country,unionid,privilege ) {
      ...
      let param = {
        ...
        sex: sex, //用户头像
        language: language, //用户头像
        city: city, //用户头像
        province: province, //用户头像
        country: country, //用户头像
        unionid: unionid //用户头像
        // privilege: newPrivilege //用户头像
      };
      let newPrivilege = [];
      ...
      that.axios({
          ...
          url: this.apis.theme.api.me.wechatLogin,
        ...
        .then(res => {
          ...
          if (res.data.status == "1") {
            ...
            //设置
            res.data.data['loginType'] = 2;
            res.data.data['params'] = param;
            that._setStorage(JSON.stringify(res.data.data));
            ...
            this.loginUpdate({  //登录成功获取积分
              userId: JSON.parse(this.getStorge).user_id,
    ...
    qqLogin( qqParam ){
      ...
      console.log( that )
      console.log( qqParam )
      console.log( JSON.stringify(qqParam) )
      // alert( JSON.stringify(qqParam) )
        that.axios({
          method: "post",
          //this.apis.theme.api.me.wechatLogin
          url: that.apis.theme.api.me.qqLogin,
          data: qs.stringify(qqParam)
        })
        .then( res => {
          ...
          if (res.data.status == "1") {
            ...
            //设置
            res.data.data['loginType'] = 3;
            res.data.data['params'] = qqParam;
            that._setStorage(JSON.stringify(res.data.data));
            localStorage.removeItem('index_Column'); //切换至其他用户，需要重置栏目
            localStorage.removeItem('removeChannel');
            localStorage.removeItem('old_index_Column');
    ...
    weiboLogin(weiboParam){
      // alert( JSON.stringify(weiboParam) )
      let that = this;
      console.log( that )
      console.log( weiboParam )
      console.log( JSON.stringify(weiboParam) )
      if( plus.os.name == 'iOS' ){
         weiboParam['openid'] = weiboParam.id;
      }
      weiboParam.headimgurl = weiboParam.avatar_hd;
        that.axios({
          method: "post",
          //this.apis.theme.api.me.wechatLogin
          url: that.apis.theme.api.me.weiboLogin,
          data: qs.stringify(weiboParam)
        })
        .then( res => {
          console.log(JSON.stringify(res.data.data));
          // alert(JSON.stringify(res.data));
          if (res.data.status == "1") {
            ...
            //设置
            res.data.data['loginType'] = 4;
            res.data.data['params'] = weiboParam;
            that._setStorage(JSON.stringify(res.data.data));
            localStorage.removeItem('index_Column'); //切换至其他用户，需要重置栏目
            localStorage.removeItem('removeChannel');
            localStorage.removeItem('old_index_Column');
    ...
    landing() {
      ...
      if (this.userPwd) {
        this.axios({
          ...
          url: this.apis.theme.api.me.loginByPassword,
          data: qs.stringify({
            mobilePhone: this.phone, //账号
          ...
          .then(res => {
            if (res.data.status == "1") {
              ...
              res.data.data['params'] = { 'mobilePhone':that.phone,'password':that.userPassword };
        ...
        console.log(this.apis.theme.api.me.loginByCaptcha);
        this.axios({
          method: "post",
          url: this.apis.theme.api.me.loginByCaptcha,
          data: qs.stringify({
            mobilePhone: this.phone, //账号
