<template>
  <!-- 热门 -->
  <div class="mintlist-tab-container-item">
    <header class="com_header headerNavFixed">
      <div class="nvbt iback" @click="apis.iback()"></div>
      <div class="nvtt" style="width:70%" >我的{{ navTitle }}</div>
    ...
    <div
      ...
      infinite-scroll-immediate-check="true" style="padding-top:1rem"
    >
      <mt-loadmore :top-method="loadTop" :auto-fill="true" ref="loadmore" :bottom-all-loaded="false" @top-status-change="handleTopChange">
        ...
        <div class="item border_1px_b" v-for="( item,index ) in json" :key="index">
          ...
          <div class="delImg"><img src="./images/del.png" alt="" @click="delready(index)"></div>
          </div>
        ...
        ></li>
         ...
         <template v-if="item.reportInfoVideos">
          ...
          <div class="listsamll" v-if="json.length==0">你还没发布内容的！</div>
      </mt-loadmore>
...
export default {
  ...
  mounted() {
    this.Init("top");
    $(".mint-loadmore").removeAttr("class")
  },
  computed: {
    ...mapGetters("login", [
      "getStorge" // active的栏目
  ...
  methods: {
   ...
   delready(index){
       let that  = this;
        // plus.nativeUI.confirm("确定删除这条爆料吗？", function(e) {
        // if (e.index == 0) {
            let param = {
              appInfoId:parseInt(this.apis.theme.api.appInfoId),
              userId: JSON.parse(localStorage.getItem( 'userData' )).user_id,
              autoId:this.json[index].autoId
            };
            axios({
              method: "post",
              url: that.apis.theme.api.me.delReportMaterial,
              data: param
            }).then(res => {
              if (res.data.success) {
...
.com_content {
  ...
  .item {
    ...
    .msg {
      ...
      .user {
        ...
        span {
          font-size: 0.28rem;
          color: #989898; display: block; margin-bottom: 0.1rem
        }
      }
      span {
        font-size: 0.28rem;
 ...
 .article_video {
        ...
        .flxesnews{flex-direction: column;display: flex;}
            .delImg{
                width: 1.36rem;
                height:0.38rem;
                margin-top: 0.15rem;
