<template>
    <!-- <swiper :options="swiperOption" class="swiper-box" ref="mySwiper" :class="{isIOS: $store.state.device == 'ios'}"> -->
        <!-- <swiper-slide v-for='(item,index) in livesColumn' :key='index' class="swiper-slide-box"> -->
            <div class="swiper-box" :class="{isIOS: $store.state.device == 'ios'}" style="padding-top:1rem">
                <div style="padding: 0 .3rem;" v-if="apis.getTheme() == 'haicang'">
                    <img src="./../images.png" alt="" style="width: 100%;height: auto;" @click="apis.urlParams('http://xmhclndx.lndxpt.com/lndxwx/lndxreg!tozzpt.do?compCode=xmhclndx',{'title':'老年大学','isWechat':true},true)">
                </div>
                <pullContainer :type='"wechatItem"'></pullContainer> 
            </div>
...
export default {
    ...
    mounted() {
        this.type=JSON.parse(get_local_cache('wechatItem')).uid;
    }
}
</script>
