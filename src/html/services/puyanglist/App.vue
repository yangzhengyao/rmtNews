<template>
 ...
 <div class="activtyall" v-if="title=='农名工之家'">
       <!-- <div class="activty-nav">
           <ul>
             <li><span :class="{active:type==1}" @click="tabClick(1)">找工作</span></li>
             <li><span :class="{active:type==0}" @click="tabClick(0)">涨经验</span></li>
           </ul>
       </div> -->
	   </div>
    </header>
...
export default {
  ...
  methods: {
        ...
        //         if(type == '1'){//进行中
        //             this.type = 1;
        //            var uuids=JSON.parse(get_local_cache('puyangall'));
        //              uuids.uid='5d55250fe4b08f7738231eba';
        //             set_local_cache('puyangall', JSON.stringify(uuids));
  ...
  mounted() {
    this.title=JSON.parse(get_local_cache('puyangall')).title;
  }
  // created() {
  //     this.init()
  // },
...
.activty-nav ul li{ color: #999999; font-size:0.36rem;width:49.9%;  text-align:center; line-height:0.76rem; float: left; }
