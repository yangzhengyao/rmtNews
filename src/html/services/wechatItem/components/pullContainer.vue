...
export default {
    ...
    methods: {
        ...
        init () {
            if(this.apis.getTheme() == 'youyu'){
                this.name=JSON.parse(get_local_cache('wechatItem')).title;
            }else{
                this.name=JSON.parse(get_local_cache('wechatItem')).name;
            }
        ...
        loadTopAjax () {
            ...
            this.classPage=0
            // this.loading = true
            this.wechat_listItem_data({page: this.classPage, name: this.name})
            .then(res => {
                this.loading = false
                var tempArr=[];
                ...
                res.data.forEach(function (item,i) {
                    tempArr.push({
                        ...
                        ptitlepic: (i%5==0&&item.picture)?'http://wxservice.wengetech.com/wenhaiFt/picture/picTrans?picUrl='+item.picture:false,
                        // titlepic: item.pictures.length>1?item.pictures[Math.floor(item.pictures.length/2)]:false,
                        titlepic: item.picture?'http://wxservice.wengetech.com/wenhaiFt/picture/picTrans?picUrl='+item.picture:false,
                        readNum: -1,
                        weixin: true,
                        columnName:item.author,
