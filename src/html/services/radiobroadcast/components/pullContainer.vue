<template>
    ...
    <div class="container swiper-box-special" :class="type" v-infinite-scroll="loadBottomAjax" infinite-scroll-disabled="bottomLock" infinite-scroll-distance="10" infinite-scroll-immediate-check="false">
        ...
        <mt-loadmore :top-method="loadTopAjax" @top-status-change="handleTopChange" ref="loadmore" :auto-fill='false' :distance='indexSwiper'>
            ...
            <!-- listItem --> 
            <youyulist :itemJsonlist="contentJson"  :radiobroadcastid="radiobroadcast" v-if='contentJson.length > 0'></youyulist>
            <!-- 底部提示 -->
...
export default {
    ...
    data () {
        return {
            ...
            xuexiFlag: {},
            childTabList:[],
            radiobroadcast:'5d031292e4b0ed2aef82fda8'
        }
    },
    ...
    methods: {
        ...
        loadTopAjax () {
            ...
            var that=this;
            this.classPage=0
            var uuid=(this.indexActive==apis.theme.news.columnId.kanTV)?this.type:null
            // this.loading = true
            this.get_listItem_data({page: this.classPage, uuid: '5d031292e4b0ed2aef82fda8'})
            .then(res => {
                // debugger
                this.loading = false
                var tempArr=[];
