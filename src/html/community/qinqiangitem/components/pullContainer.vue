...
<script>
import { formatDate, getDateDiff, get_local_cache } from '../../../../../src/assets/js/cache.js'
...
export default {
    ...
    methods: {
        ...
        ...mapActions('qinqiang', [
            'get_listItem_cache',
            'get_listItem_data',
            'qinqiang_listItem_data'
        ]),
        init () {
            this.uuid=JSON.parse(get_local_cache('./community/qinqiangitem.html')).uuid;
        ...
        loadTopAjax () {
            ...
            this.classPage=0
            this.loading = true
            this.qinqiang_listItem_data({page: this.classPage, uuid: this.uuid,userId:this.userDataid})
            .then(res => {
                this.loading = false
                var tempArr=[];
