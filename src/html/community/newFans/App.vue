      ...
      <div class="friendsContent">
        ...
        <div class="item border_1px_b">
          ...
          <div>
            <div class="my_follow">
              <span>我是标题</span>
              <button class="select_follow">
                <img src="./images//xingzhuang 2.png" alt />
                <span>互相关注</span>
              </button>
            </div>
            <span>我是描述</span>
        ...
        <div class="item border_1px_b" v-for="(item,index) in json " :key="index">
          ...
          <div class="myBox">
            ...
            <div class="focuses" :class="item.focus?'select_follow':''" @click="clickFollow(index,item.focus,item.autoId)">
                ...
                <p v-if="item.focus == 0">未关注</p>
                <p v-if="item.focus == 1">已关注</p>
                <p v-if="item.focus == 2">互相关注</p>
              </div>
          </div>
        </div>
      </div>
  ...
  methods: {
    ...
    initUserMsg() {
      ...
      let postData = {
        pageNum: 1,
        pageSize: 10,
        userId: that.initParams.id
      };
      that.noLoading = true;
      this.axios({
        method: "post",
        url: that.apis.theme.api.community.getFansList,
        data: postData
      }).then(res => {
        if (res.data.success ) {
    ...
    clickFollow(index, status, user_id) {
      ...
      let postData = { 
          userId: user_id,
          fansId:JSON.parse( that.getStorge ).user_id
        }; 
      that.axios({
        method: "post",
        url: url,
