...
export default {
    ...
    methods: {
        ...
        shareAction(sb) {
            ...
            if(that.type){
                msg.href = "http://hongqi.wengetech.com:28086/app/app_share/app_share.html?id="+that.json.auto_id+'&appInfoId='+that.json.app_info_id;
                msg.title = that.json.content;
                msg.content = that.json.user.personName+'发布了一条动态，快来看看吧！';
                if(sb.s.id == 'sinaweibo' && plus.os.name=='Android'){
                    msg.thumbs = ['_www/logo.png']
                    msg.pictures = ['_www/logo.png']
                }else {
                    // if(that.json.blog_info_images){
                    //     let img = JSON.parse(that.json.blog_info_images);
                    //     if(img.length > 0){
                    //         msg.thumbs = [img.images];
                    //         msg.pictures = [img.images];
                    //     }else{
                    //         msg.thumbs = ['_www/logo.png']
                    //         msg.pictures = ['_www/logo.png']
                    //     }
                ...
                if(sb.s.id == 'sinaweibo' && plus.os.name=='Android'){
                    ...
                    msg.thumbs = shareData.img_cut_url?[shareData.img_cut_url]:['_www/logo.png'];
                    msg.pictures = shareData.img_cut_url?[shareData.img_cut_url]:['_www/logo.png'];
                }
            }
        ...
        shareMessage(msg, s) {
            ...
            if(!this.type){
                let param = JSON.parse( localStorage.getItem('history_Article'))[0];
                if(this.userDataid){
                    addData = {type: 2, columnName: param.id,userId:this.userDataid}
                }else{
                    addData = {type: 2, columnName: param.id,userId:''}
                }
