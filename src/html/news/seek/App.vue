<template>
  <div id="Serarch">
    <header class="com_header headerNavFixed" style="background:#fff">
      <div class="nvbt iback ibackBack" @click="apis.iback()" style="padding-top: .1rem;background-position-y: .32rem;"></div>
      <div class="nvtt seekNvtt" style="position:relative">
        <input class="form_input" type="text" placeholder="搜索您感兴趣的内容" v-model.trim="key" style="background:#F5F5F5"/>
        <img src="./serarch.png" alt="">
        <img src="./del.png" alt="" v-if="search_del" @click="delFn">
      </div>
      <div class="nvbt idoc" style="font-size:.32rem;color:#1A1A1A;padding-top: .06rem;" @click="Serarch">搜索</div>
    </header>
    <div class="historySerarch" v-if="serarchHistory.length > 0" v-show="!serarchHistoryShow" :class="iphonexPadding ? 'iphonexPaddingClass':'androidPaddingClass' ">
        <div v-for="(item,index) in serarchHistory" :key="index" >
...
export default {
  ...
  watch: {
    ...
    key(val) {
      ...
      if( val ){
        this.search_del = true;
      } else {
        this.search_del = false;
      }
    }
  },
  methods: {
    ...
    clickItem(index){
        console.log( index )
        this.serarchHistory.splice(index,1);
        localStorage.setItem('serarchHistory',JSON.stringify( this.serarchHistory ))
    },
    clear(){
        localStorage.removeItem('serarchHistory');
        this.serarchHistory = [];
    },
    ...
    Serarch() {
      ...
      if (this.key == "") {
        ...
        that.serarchHistory.forEach((v,i) => { //每当搜索一个重复的值，应删除重复老值
          if(v.keywords == that.key){
            that.serarchHistory.splice(i,1);
          }
        })
        if( that.serarchHistory.length >= 5 ){
           that.serarchHistory.unshift(
                {
                    keywords:that.key
                }
            )
           that.serarchHistory.pop();
        }else {
          console.log( '小于5' )
           that.serarchHistory.unshift(
                {
        ...
        localStorage.setItem('serarchHistory',JSON.stringify( that.serarchHistory ))
        this.serarchHistoryShow = true;
        this.loadTopAjax();
      }
    },
  ...
  mounted() {
    if( localStorage.getItem('serarchHistory') != null ){
        this.serarchHistory = JSON.parse( localStorage.getItem('serarchHistory') );
    }
