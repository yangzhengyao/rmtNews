...
export default {
    ...
    actions: {
     ...
     // 铁门关法制新闻子级tab
     async lists_child_listItem_data({ rootState, getters }) {
        let params = {
            app_key: 'sprint-ucm',
            app_secret: 'bbbbbbaaaaaaaaaaaaa',
        }
        let res = await fetch('post', 'classID', {appInfoId: apis.theme.api.appInfoId, parentId: "5d24707be4b09067cf75240c"}, params)
        return res
    },
        // 获取栏目数据
        async get_indexColumn_data({ commit, state, dispatch }) {
            ...
            // } else {
                ...
                let data = await fetch('post', 'classID', {appInfoId: apis.theme.api.appInfoId, parentId: apis.theme.wenlv.parentId.qinqiang}, params)
                let json = []
                let json1 = []
                let jsonT = []
