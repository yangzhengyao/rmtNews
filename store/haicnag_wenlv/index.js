...
export default {
    ...
    state: {
        ...
        // }],
        indexActive: apis.theme.hcChild.columnIdOrder[0],          // active的栏目
    ...
    mutations: {
        ...
        set_indexColumn(state, arr) {
            ...
            set_local_cache('haicang_wenlv_Column', arr)
        },
    ...
    actions: {
        ...
        async get_indexColumn_data({ commit, state, dispatch }) {
            ...
            // } else {
                ...
                let data = await fetch('post', 'classID', {appInfoId: apis.theme.api.appInfoId,parentId: apis.theme.hclv.parentId.qinqiang}
                // {
                //     type: 'column',
                //     parentId: apis.theme.hclv.parentId.qinqiang
                // }
                , params)
                let json = []
                let json1 = []
                ...
                data.data.forEach(function (item, i) {
                    if (item.uuid == apis.theme.hcChild.columnIdOrder[0]) {
                        jsonT.push({
                            'classid': item.uuid,
                            'classname': item.name,
                            'classpath': item.uuid,
                    ...
                    } else if (item.uuid == apis.theme.hcChild.columnIdOrder[1]) {
                        json1.unshift({
                            'classid': item.uuid,
                            'classname': item.name,
                            'classpath': item.uuid,
                    ...
                    } else if (item.uuid == apis.theme.hcChild.columnIdOrder[2]) {
                        json1.push({
                            'classid': item.uuid,
                            'classname': item.name,
                            'classpath': item.uuid,
                    ...
                    // else if (item.uuid == apis.theme.hcChild.columnIdOrder[3]) {
                    //     json.unshift({
                    //         'classid': item.uuid,
                    //         'classname': item.name,
                    //         'classpath': item.uuid,
        ...
        async get_listItem_data({ rootState, getters }, {page, uuid,userId}) {
            ...
            //     'page': page,
            //     'size': 10,
            //     'columnId':uuid?uuid:getters.activeClassid!=''?getters.activeClassid:apis.theme.hclv.columnIdOrder[0]
            // }
            let params = {
                'pageNum': page,
                'pageSize': 10,
        ...
        // 文旅子级tab
        async hcwenlv_child_listItem_data({ rootState, getters }) {
            let params = {
                app_key: 'sprint-ucm',
                app_secret: 'bbbbbbaaaaaaaaaaaaa',
            }
            let res = await fetch('post', 'classID',{appInfoId: apis.theme.api.appInfoId,parentId: apis.theme.hcChild.columnIdOrder[1]}
            //  {
            //         type: 'column',
            //         parentId: apis.theme.hcChild.columnIdOrder[1]
            //     }
                , params)
            return res
        },
        // 秦腔列表
        async qinqiang_listItem_data({ rootState, getters }, { page, uuid}) {
            console.log(uuid)
            // let params = {
            //     'id': uuid,
            //     'app_key': 'sprint-ucm',
            ...
            let res = await fetch('post', 'Class', params,{'page': page}, )
            return res
        },
