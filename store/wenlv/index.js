...
export default {
    ...
    state: {
        ...
        // }],
        indexActive: apis.theme.wenlv.columnIdOrder[0],          // active的栏目
    ...
    mutations: {
        ...
        set_indexColumn(state, arr) {
            ...
            set_local_cache('wenlv_index_Column', arr)
        },
    ...
    actions: {
        ...
        async get_indexColumn_data({ commit, state, dispatch }) {
            let res
            // const data = JSON.parse(get_local_cache('wenlv_index_Column'))
            // console.log(data)
            ...
            // } else {
                ...
                let data = await fetch('post', 'classID', {appInfoId: apis.theme.api.appInfoId,parentId: apis.theme.wenlv.parentId.wenlv},
                // {
                //     type: 'column',
                //     parentId: apis.theme.wenlv.parentId.wenlv
                // },
                 params)
                ...
                data.data.forEach(function (item, i) {
                    if (item.uuid == apis.theme.wenlv.columnIdOrder[0]) {
                        jsonT.push({
                            'classid': item.uuid,
                            'classname': item.name,
                            'classpath': item.uuid,
                    ...
                    } else if (item.uuid == apis.theme.wenlv.columnIdOrder[1]) {
                        json1.unshift({
                            'classid': item.uuid,
                            'classname': item.name,
                            'classpath': item.uuid,
                    ...
                    } else if (item.uuid == apis.theme.wenlv.columnIdOrder[2]) {
                        json1.push({
                            'classid': item.uuid,
                            'classname': item.name,
                            'classpath': item.uuid,
                    ...
                    } else if (item.uuid == apis.theme.wenlv.columnIdOrder[3]) {
                        json.unshift({
                            'classid': item.uuid,
                            'classname': item.name,
                            'classpath': item.uuid,
                ...
                //     'showclass': '0'
                // }]
                // json = jsonT.concat(json1, json, list)
        ...
        async wenlv_child_listItem_data({ rootState, getters }) {
            ...
            let res = await fetch('post', 'classID', {appInfoId: apis.theme.api.appInfoId,parentId: apis.theme.wenlv.parentId.lvyoujingqu},
            // {
            //         type: 'column',
            //         parentId: apis.theme.wenlv.parentId.lvyoujingqu
            // },
            params)
            return res
        },
        ...
        // 闻海专题列表
        async displayConfigs_listItem_data({ rootState, getters }, page) {
            let params = {
                userId: 112,
                analysisCategory: '主题'
            }
            let res = await fetch('get', 'Class4', params)
            return res
        },
